...
class Classifier(object):
    ...
    def load_model(self):
        ...
        self.best_saved_model_name = '/home/qizhoupeng/python workspace/result/{}_epoch_{}_ckpt.t7'.format(model_details.gpu,self.model_name_str)
        ...
        try:
            # Load checkpoint.
            assert (os.path.isdir('/home/qizhoupeng/python workspace/result'), 'Error: no checkpoint directory found!')
    ...
    def train(self, epoch):
        d = {"whetherdamage": 0, "damagelevel": 0}
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            # correct += int(predicted.eq(targets.data)[0])
        #     print(correct/total)
        # print(correct/total)
        for batch_idx, (inputs, targets) in enumerate(self.testloader):
            ...
            if int(predicted[0]) == 0:
                model1 = torch.load("/home/qizhoupeng/python workspace/result/2.t7")['model']
                model1.train()
                fi = glob.glob(os.path.join("", "/home/qizhoupeng/car damage/car-damage-dataset/data3a/training/**/**.JPEG"))
                da = data_reader.get_data(fi)
                np.random.shuffle(da)
                train_data_set = data_reader.DatasetReader(da, "train")
                tloader = torch.utils.data.DataLoader(train_data_set, batch_size=1, shuffle=True)
                c = 0
                t = 0
                for batch_idx, (inputs, targets) in enumerate(tloader):
                    # if batch_idx > 10:
                    #     break
                    step = epoch * len(self.trainloader) + batch_idx
                    # if not self.augment_images==None:
                    ...
                    if batch_idx % 5 == 0:
                        self.writer.add_scalar('step loss', batch_loss, step)
                    # t += targets.size(0)
                    # c += int(predicted.eq(targets.data)[0])
                    # print(c/t)
                for batch_idx, (inputs, targets) in enumerate(self.testloader):
                    # if batch_idx > 10:
                    #     break
    ...
    #         'epoch': epoch,
    #     }
    #     if not os.path.isdir('/home/qizhoupeng/python workspace/result'):
    #         os.mkdir('/home/qizhoupeng/python workspace/result')
    ...
    def create_submit_file(self,type,model,epoch,loader_for_upload):
        ...
        with torch.no_grad():
            print(enumerate(loader_for_upload))
            for batch_idx, (inputs) in enumerate(loader_for_upload):
                ...
                for index in range(start_index, end_index):
                    ...
                    score_row = "{id},{:.3f},{:.3f}".format(id=image_id, *probs)
    ...
    def test(self, epoch):
        ...
        epoch_saved_model_name = '/home/qizhoupeng/python workspace/result/{}_epoch_{}_ckpt.t7'.format(gpu, self.model_name_str, epoch)
